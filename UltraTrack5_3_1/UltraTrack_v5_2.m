function varargout = UltraTrack_v5_2(varargin)
% ULTRATRACK_V5_2 M-file for UltraTrack_v5_2.fig
%      ULTRATRACK_V5_2, by itself, creates a new ULTRATRACK_V5_2 or raises the existing
%      singleton*.
%      H = ULTRATRACK_V5_2 returns the handle to a new ULTRATRACK_V5_2 or the handle to
%      the existing singleton*.
%
%      ULTRATRACK_V5_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ULTRATRACK_V5_2.M with the given input arguments.
%
%      ULTRATRACK_V5_2('Property','Value',...) creates a new ULTRATRACK_V5_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before UltraTrack_v5_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to UltraTrack_v5_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help UltraTrack_v5_2

% Last Modified by GUIDE v2.5 02-May-2022 10:36:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @UltraTrack_v5_2_OpeningFcn, ...
    'gui_OutputFcn',  @UltraTrack_v5_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before UltraTrack_v5_2 is made visible.
function UltraTrack_v5_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to UltraTrack_v5_2 (see VARARGIN)

% Choose default command line output for UltraTrack_v5_2
...
% UIWAIT makes UltraTrack_v5_2 wait for user response (see UIRESUME)
...
function varargout = UltraTrack_v5_2_OutputFcn(~, eventdata, handles)
...
if isfield(handles,'ImStack')
    ...
    if ii == 1 || 2
        ...
        imshow(handles.IM); hold on;

        %% Superficial aponeurosis
        [~,supAp] = ginputYellow(1);

        % filter image
        %im = vesselness2D(handles.IM, 18:20, [1;1], 1, true);
        im = handles.IM;
        imAp = imbinarize(im,'adaptive');

        % crop image
        handles.ID = handles.ID*size(handles.Im,1);
        ap_sup = imcrop(imAp,[1 supAp-2/(handles.ID/size(handles.Im,1)) size(handles.Im,2)-1 2/(handles.ID/size(handles.Im,1))*2-1]);
        ap_sup = bwpropfilt(ap_sup,'MajorAxisLength',1);
        ap_fill_top = false(round(supAp-2/(handles.ID/size(handles.Im,1))-1,0),size(handles.Im,2));
        ap_fill_bot = false(round(size(handles.Im,1)-supAp-2/(handles.ID/size(handles.Im,1))-1,0),size(handles.Im,2));
        ap_sup = [ap_fill_top; ap_sup; ap_fill_bot];

        % fit whole apo
        [ap_sup_coef,~,ap_sup_y] = obj2ap_new(ap_sup,1);
        ptS(:,1) = 1:size(handles.Im,2);
        ptS(:,2) = polyval(ap_sup_coef,ptS(:,1));
        %ptS(:,2) = ptS(:,2)-10;
        plot(ptS(:,1),ptS(:,2),'y:','LineWidth',2)

        %% added
        hf=figure; imshow(handles.IM); hold on;
        h = imline(gca,[ptS(1,1) ptS(1,2); ptS(end,1) ptS(end,2)]);
        ptS = wait(h);
        close(hf);
        axes(handles.axes1)
        plot(ptS(:,1),ptS(:,2),'c:','LineWidth',2)
        ap_sup_coef = polyfit(ptS(:,1),ptS(:,2),1);

        %% Deep aponeurosis
        [~,deepAp] = ginputYellow(1);

        % crop image as it's already filtered from the sup. apo determination
        ap_deep = imcrop(imAp,[1 deepAp-4/(handles.ID/size(handles.Im,1)) size(handles.Im,2)-1 4/(handles.ID/size(handles.Im,1))*2-1]);
        ap_deep = bwpropfilt(ap_deep,'MajorAxisLength',1);
        ap_fill_top = false(round(deepAp-4/(handles.ID/size(handles.Im,1))-1,0),size(handles.Im,2));
        ap_fill_bot = false(round(size(handles.Im,1)-deepAp-4/(handles.ID/size(handles.Im,1))-1,0),size(handles.Im,2));
        ap_deep = [ap_fill_top; ap_deep; ap_fill_bot];
        [ap_deep_coef,~,ap_deep_y] = obj2ap_new(ap_deep,2,2);
        ptD(:,1) = linspace(1,size(handles.Im,2)); %,2
        ptD(:,2) = polyval(ap_deep_coef,ptD(:,1));
        %ptD(:,2) = ptD(:,2)+27;

        ap_deep_coef2 = polyfit([ptD(1,1) ptD(end,1)],[ptD(1,2) ptD(end,2)],1);
        if isnan(ap_deep_coef(1,1))
            [ap_deep_coef,~,ap_deep_y] = obj2ap_new(ap_deep,2);
            ptD(:,2) = polyval(ap_deep_coef,ptD(:,1));
            ap_deep_coef2 = polyfit(ptD(:,1),ptD(:,2),1);
        end
        axes(handles.axes1)
        plot(ptD(:,1),ptD(:,2),'y:','LineWidth',2)

        hf=figure; imshow(handles.IM); hold on;
        h = imline(gca,[ptD(1,1) ptD(1,2); ptD(end,1) ptD(end,2)]);
        ptD = wait(h);
        close(hf);
        axes(handles.axes1)
        plot(ptD(:,1),ptD(:,2),'c:','LineWidth',2)
        ap_deep_coef2 = polyfit(ptD(:,1),ptD(:,2),1);

        %% fascicle selection new
%         %[~,Fas] = ginputYellow(1);
%         im = handles.IM;
%         imAp = imbinarize(im,'adaptive');
% 
%         % crop image
%         fasBW = imcrop(imAp,[1 max(ptS(:,2)) size(handles.Im,2)-1 min(ptD(:,2))-max(ptS(:,2))]);
%         fasBW = bwpropfilt(fasBW,'MajorAxisLength',1);
%         fasBW_fill_top = false(round(max(ptS(:,2))-1,0),size(handles.Im,2));
%         fasBW_fill_bot = false(round(size(handles.Im,1)-max(ptS(:,2))-1,0),size(handles.Im,2));
%         fasBW = [fasBW_fill_top; fasBW; fasBW_fill_bot];
% 
%         % fit fas
%         fasF = obj2ap_new(fasBW,1);
%         data(:,1) = 1:size(handles.Im,2);
%         data(:,2) = polyval(fasF,data(:,1));
%         %ptS(:,2) = ptS(:,2)-10;
%         plot(data(:,1),data(:,2),'y:','LineWidth',2)
%         hf=figure; imshow(handles.IM); hold on;
%         h = imline(gca,[data(1,1) data(1,2); data(end,1) data(end,2)]);
%         data = wait(h);
%         close(hf);
%         axes(handles.axes1)
%         plot(data(:,1),data(:,2),'c:','LineWidth',2)

        %% fascicle selection with feature points
            % crop image
            imF = imcrop(handles.IM,[1 max(ptS(:,2)) size(handles.Im,2) min(ptD(:,2))-max(ptS(:,2))]);
        
            % define fascicle by user-selected points
            if isempty(imF)
                imF = imcrop(handles.IM,[1+1/(handles.ID/size(handles.Im,1)) max(ap_sup_y)+2/(handles.ID/size(handles.Im,1)) size(handles.Im,2)-1/(handles.ID/size(handles.Im,1))-1 min(ap_deep_y)-0.4/(handles.ID/size(handles.Im,1))-max(ap_sup_y)-1]);
            end
        
            %first set of points
            detect1_im = detectKAZEFeatures(imF);
            detect1 = detect1_im.selectStrongest(100);
            points1 = detect1.Location;
            [~,idx1] = sort(points1(:,1));
            points1 = points1(idx1,:);
            points1(:,2) = points1(:,2)+max(ptS(:,2))-1;
        
            %show points
            figure; set(gcf,'Name','Select fascicle features to track, remove unbrushed and then confirm','Numbertitle','off'); imshow(handles.IM); set(gcf,'WindowState','maximized'); hold on;
            hPts = plot(points1(:,1),points1(:,2),'y+');
        
            % select points
            brush on
            title('Select fascicle features to track, remove unbrushed and then confirm')
            hPos = get(gcf);
        ...
        %% fit fascicle
        fasF = polyfit(data(:,1),data(:,2),1);
        fas(:,1) = 1:size(handles.Im,2);
        fas(:,2) = polyval(fasF,fas(:,1),1);
        ...
        [handles.Region(i).ROI{frame_no},handles.Region(i).ROIx{frame_no}, handles.Region(i).ROIy{frame_no}] = roipoly(handles.IM,vertcat(ptD(1,1),size(handles.Im,2),ptS(end,1),1,ptD(1,1)),vertcat(ptD(1,2),ptD(2,2),ptS(end,2),ptS(1,2),ptD(1,2))); %ptS(end,2)-20,ptS(end,2)-20,ptD(1,2)));
        x_int(1,1) = double(fzero(@(x) polyval(ap_sup_coef-fasF,x),3));
        x_int(1,2) = double(fzero(@(x) polyval(ap_deep_coef2-fasF,x),3));
        y_int(1,1) = double(polyval(ap_sup_coef,x_int(1,1)));
        y_int(1,2) = double(polyval(ap_deep_coef2,x_int(1,2)));
        handles.Region(i).Fascicle(j).fas_x{frame_no}(1) = x_int(1,1);
        handles.Region(i).Fascicle(j).fas_x{frame_no}(2) = x_int(1,2);
        handles.Region(i).Fascicle(j).fas_y{frame_no}(1) = y_int(1,1);
        handles.Region(i).Fascicle(j).fas_y{frame_no}(2) = y_int(1,2);
        ...
        for i = 1:size(handles.Region,2)
            ...
            if i == 1
                scalar = handles.ID/size(handles.Im,1);
                handles.CIm = handles.Im;
            end
