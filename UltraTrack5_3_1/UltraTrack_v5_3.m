function varargout = UltraTrack_v5_3(varargin)
% ULTRATRACK_V5_3 M-file for UltraTrack_v5_3.fig
%      ULTRATRACK_V5_3, by itself, creates a new ULTRATRACK_V5_3 or raises the existing
%      singleton*.
%      H = ULTRATRACK_V5_3 returns the handle to a new ULTRATRACK_V5_3 or the handle to
%      the existing singleton*.
%
%      ULTRATRACK_V5_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ULTRATRACK_V5_3.M with the given input arguments.
%
%      ULTRATRACK_V5_3('Property','Value',...) creates a new ULTRATRACK_V5_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before UltraTrack_v5_3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to UltraTrack_v5_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help UltraTrack_v5_3

% Last Modified by GUIDE v2.5 08-Jul-2022 13:49:26
% Last Modified by Paolo Tecchio 17/08/2022
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @UltraTrack_v5_3_OpeningFcn, ...
    'gui_OutputFcn',  @UltraTrack_v5_3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before UltraTrack_v5_3 is made visible.
function UltraTrack_v5_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to UltraTrack_v5_3 (see VARARGIN)

% Choose default command line output for UltraTrack_v5_3
...
% UIWAIT makes UltraTrack_v5_3 wait for user response (see UIRESUME)
...
function varargout = UltraTrack_v5_3_OutputFcn(~, eventdata, handles)
...
if isfield(handles,'ImStack')
    ...
    if ii == 1 || 2
        ...
        data = img(handles.crop_rect(2):(handles.crop_rect(2)+handles.crop_rect(4)), handles.crop_rect(1):(handles.crop_rect(1)+handles.crop_rect(3)));
        ...
        parms.apo.super.cut = [max(apRound(1)-10, 0), apRound(1)+10] / 100;
        parms.apo.deep.cut = [apRound(2)-10, min(apRound(2)+10, 100)] / 100;
        
        
        [fas_thres, super_obj, deep_obj] = filter_usimage(data,parms);
        
        polipo = drawpolygon('Color','g' , 'InteractionsAllowed','all', 'Position',[parms.apo.apox flip(parms.apo.apox); super_obj flip(deep_obj)]');
        
        fasmask = poly2mask(polipo.Position(:,1), polipo.Position(:,2), m, n);  
        fasimage = fasmask .* data;
        
        anglerange = sort(90-parms.fas.range);
        fasangles = anglerange(1):parms.fas.thetares:anglerange(2);
        
        [hmat,theta,rho] = hough(fasimage,'RhoResolution',parms.fas.rhores,'Theta',fasangles);
        
        P = houghpeaks(hmat, parms.fas.npeaks,'Threshold',0);
     
        [y,x] = find(hough_bin_pixels(fasimage, theta, rho, P(1,:)));

        
        figure(5)
        imshow(data);
        polipo = drawpolygon('Color','g' , 'InteractionsAllowed','all', 'Position',[parms.apo.apox flip(parms.apo.apox); super_obj flip(deep_obj)]');
        line('xdata',x,'ydata',y ,'linestyle','none','marker','.', 'color', 'Red');

        
        % Identify aponeurosis ROIs and lines
         apoFrangiFilt      = struct('FrangiScaleRange', [1/handles.ID 2*(1/handles.ID)], 'FrangiScaleRatio', 1, ...
        ...
        refImageFiltered  = FrangiFilter2D(img, apoFrangiFilt);
%        refImageFiltered = vesselness2D(img, 10, [1;1], 1, true);
        refImageFiltered  = imbinarize(refImageFiltered, 'adaptive', 'ForegroundPolarity', 'bright',  'sensitivity', .2);

        [apo1.line apo1.ROI] = getAp(handles, refImageFiltered, apo1searchWin .* [handles.vidHeight handles.vidHeight]);
        [apo2.line apo2.ROI] = getAp(handles, refImageFiltered, apo2searchWin .* [handles.vidHeight handles.vidHeight]);
        ...
        fasROI.position = [         1              apo1.line(1,2)+(1/handles.ID); ...
            handles.vidWidth-1  apo1.line(2,2)+(1/handles.ID); ...
            handles.vidWidth-1  apo2.line(2,2)-(1/handles.ID); ...
            1              apo2.line(1,2)-(1/handles.ID)];
        fasROI.mask = poly2mask(fasROI.position(:,1), fasROI.position(:,2), handles.vidHeight, handles.vidWidth);

        [ROIr, ROIc] = find(fasROI.mask);
        fasROI.pixels = img .* fasROI.mask;
        fasROI.pixels = fasROI.pixels(min(ROIr):max(ROIr), min(ROIc):max(ROIc));

        h = figure;
        imshow(img);
        hold on;
        title('Press Done to continue');

        % Draw fascicle region of interest
        polipo = drawpolygon('Color','g' , 'InteractionsAllowed','all', 'Position',fasROI.position);

        % Draw aponeurosis lines
        apo_manual_up = drawline('Color','r' , 'InteractionsAllowed','all', 'Position', apo1.line);
        apo_manual_low = drawline('Color','r' , 'InteractionsAllowed','all', 'Position', apo2.line);
...
%         scatter(apo2.ROI.properties.Centroid(1,1), apo2.ROI.properties.Centroid(1,2), 'bo', 'LineWidth', 4);
        ...
        uicontrol('Units', 'Normalized','Position',[.7 .2 .15 .05],'String','Done',...
            'Callback','uiresume(gcf)');
        uiwait(gcf);      

        %%Get Manual adjustments      
        
        apo1.line = apo_manual_up.Position;
        apo2.line = apo_manual_low.Position;
        fasROI.position = polipo.Position;

        close(h);
        %% 
        [fasROI Hough] = detectFas(handles, fasROI, apo1, apo2, ii);
        [apo1 apo2 Hough FPT] = getFas(handles, apo1, apo2, Hough, ii);

        %% Check fascicle and manually correct in case
        hf=figure; imshow(img); hold on;
        h = imline(gca,[FPT(1).apo1int(1,1) FPT(1).apo1int(1,2); FPT(1).apo2int(1,1) FPT(1).apo2int(1,2)]);
        Fas_Final = wait(h);
        close(hf);
        
        %update points
        FPT(1).apo1int(1,1) = Fas_Final(1,1);
        FPT(1).apo1int(1,2) = Fas_Final(1,2);
        FPT(1).apo2int(1,1) = Fas_Final(2,1);
        FPT(1).apo2int(1,2) = Fas_Final(2,2);
